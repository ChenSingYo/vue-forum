    ...
    >
      <div class="row no-gutters">
        <div class="col-md-4">
          ...
          >
            <img class="card-img" :src="topRestaurant.image" alt="image" />
          </router-link>
        </div>
        <div class="col-md-8">
          <div class="card-body">
            ...
            <span class="badge badge-secondary"
              >收藏數：{{ topRestaurant.FavoriteCount }}</span
            >
            <p class="card-text">
            ...
            <button
              ...
              @click.stop.prevent="removeFavorite(topRestaurant.id)"
            >
              移除最愛
            </button>
            <button
              ...
              @click.stop.prevent="addFavorite(topRestaurant.id)"
            >
              加到最愛
            </button>
          </div>
...
export default {
  ...
  methods: {
    ...
    addFavorite (topRestaurantId) {
      // 修改資料內容
      this.topRestaurants = this.topRestaurants
        .map(topRestaurant => {
          if (topRestaurant.id !== topRestaurantId) {
            return topRestaurant
          }
          return {
            ...topRestaurant,
            FavoriteCount: topRestaurant.FavoriteCount + 1,
            isFavorited: true
          }
        })
    },
    removeFavorite (topRestaurantId) {
      // 修改資料內容
      this.topRestaurants = this.topRestaurants
        .map(topRestaurant => {
          if (topRestaurant.id !== topRestaurantId) {
